function landmark = alignLandmarksFEI(inPathList, inPathList1, inPathListh, inPathList1h, benchmark, outPath)


landmark_orig = zeros(0,132);
landmark_aligned = zeros(0,132);
fileName = cell(0);
for i=1:length(inPathListh)
    disp(['Processing ', inPathListh{i}]);
    load(inPathList{i});
    load(inPathList1{i});
    if size(bs,2) > 1
        bs = bs(1);
